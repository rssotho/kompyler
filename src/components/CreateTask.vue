...
export default {
  ...
  computed: {
    inProgressTasks() {
      try {
        ...
        // Get tasks that are pending/in-progress
    ...
    taskStats() {
      try {
        ...
        return {
          ...
          passedTasks: tasks.filter(task => task.status === 'completed').length,
          failedTasks: tasks.filter(task => task.status === 'cancelled').length,
    ...
    completionRate() {
      const { totalTasks, passedTasks } = this.taskStats;
      return totalTasks ? Math.round((passedTasks / totalTasks) * 100) : 0;
    }
  },
  methods: {
    updateClock() {
    ...
    getTaskStats(memberId) {
      try {
        ...
        // Log member's pending tasks
        console.log(`CreateTask - Member ${memberId} pending tasks:`, stats.pending);
        console.log(`CreateTask - Member ${memberId} task details:`, memberTasks);
  ...
  beforeUnmount() {
    ...
    eventBus.off('task-deleted', this.refreshData);
